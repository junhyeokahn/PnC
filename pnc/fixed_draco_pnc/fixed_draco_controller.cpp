...
FixedDracoController::FixedDracoController(FixedDracoTCIContainer *_tci_container,
                                 RobotSystem *_robot) {
...
void FixedDracoController::getCommand(void *cmd) {
  if (sp_->state == draco_states::kInitialize) {
    ...
    Eigen::VectorXd joint_acc_cmd =
        sa_.transpose() * wbc_joint_acc_cmd;
  ...
  ((FixedDracoCommand *)cmd)->joint_torques = robot_->vector_to_map(joint_trq_cmd_);
}
