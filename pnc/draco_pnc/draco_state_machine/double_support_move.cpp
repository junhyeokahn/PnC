...
DoubleSupportMove::DoubleSupportMove(
    ...
    DracoControlArchitecture *_ctrl_arch, int _com_move_states, RobotSystem *_robot)
...
void DoubleSupportMove::firstVisit() {
    if (com_move_states_ == com_move_states::Left) {
       ...
       Eigen::Vector3d target_com_pos = robot_->get_link_iso("l_foot_contact").translation(); 
       target_com_pos[2] = robot_->get_com_pos()[2];
       Eigen::Quaternion<double> target_base_quat = Eigen::Quaternion<double>(
               robot_->get_link_iso("torso_com_link").linear()); 

        ctrl_arch_->floating_base_tm->InitializeInterpolationTrajectory(sp_->curr_time, 
          duration_,target_com_pos, target_base_quat); 
    } else if (com_move_states_ == com_move_states::Right) {
       std::cout << "draco_states::kMoveCoMToRFoot" << std::endl;

       Eigen::Vector3d target_com_pos = robot_->get_link_iso("r_foot_contact").translation(); 
       target_com_pos[2] = robot_->get_com_pos()[2];
       Eigen::Quaternion<double> target_base_quat = Eigen::Quaternion<double>(
               robot_->get_link_iso("torso_com_link").linear()); 
       ...
       Eigen::Vector3d target_com_pos = 0.5 * (robot_->get_link_iso("l_foot_contact").translation()
               + robot_->get_link_iso("r_foot_contact").translation()); 
       target_com_pos[2] = robot_->get_com_pos()[2];
       Eigen::Quaternion<double> left_foot_ori(robot_->get_link_iso("l_foot_contact").linear());
       Eigen::Quaternion<double> right_foot_ori(robot_->get_link_iso("r_foot_contact").linear());
       Eigen::Quaternion<double> target_base_quat = left_foot_ori.slerp(0.5, right_foot_ori); 
