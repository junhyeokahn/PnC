...
TEST(DracoKFStateEstimatorTest, roll90Deg)
{
  // Note:
  // the magnetometer measurement obtained for Austin, TX in microTeslas
  // (https://www.ngdc.noaa.gov/geomag/calculators/magcalc.shtml#igrfwmm)
  Eigen::Vector3d gyro(0.0, 0.0, 0.0);
  Eigen::Vector3d accelerometer(0.0, 0.0, 9.8);
  Eigen::Vector3d magnetometer(24.16, 1.45, 39.98);

  Eigen::Matrix3d expectedRotMatrix;
  Eigen::Matrix3d estimatedMARGRotMatrix, estimatedIMURotMatrix;

  Eigen::AngleAxis<double> roll90(M_PI_2, Eigen::Vector3d(1., 0., 0.));
  expectedRotMatrix = roll90.toRotationMatrix();
  Eigen::Quaternion<double> rollQuat(expectedRotMatrix);

  Eigen::Vector3d rotatedAccelerometer, rotatedMagnetometer;
  rotatedAccelerometer = roll90.toRotationMatrix().inverse() * accelerometer;
  rotatedMagnetometer = roll90.toRotationMatrix().inverse() * magnetometer;

  MARGFilter *margFilter = new MARGFilter();
  margFilter->initialize(rollQuat.w(), rollQuat.x(), rollQuat.y(), rollQuat.z());
  MARGFilter *imuFilter = new MARGFilter();
  imuFilter->initialize(rollQuat.w(), rollQuat.x(), rollQuat.y(), rollQuat.z());

  margFilter->filterUpdate(gyro(0), gyro(1), gyro(2),
                           rotatedAccelerometer(0), rotatedAccelerometer(1), rotatedAccelerometer(2),
                           rotatedMagnetometer(0), rotatedMagnetometer(1), rotatedMagnetometer(2));
  estimatedMARGRotMatrix = margFilter->getBaseRotation();
