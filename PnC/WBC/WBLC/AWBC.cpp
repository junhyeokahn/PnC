...
void AWBC::EstimateExtforce(const std::vector<ContactSpec*> & contact_list)
{
    ...
    Eigen::Matrix3d rt_skew;
    rt_skew = _Skew(ext_pos_);

    // coefficient matrices // JM check here
    // Eigen::MatrixXd Zq = (2*total_m_/delta_t_)*Sf_*J_com_prev_.block(3,0,3,n) 
    //                      + St_*(1/delta_t_*H_prev_-2*total_m_/delta_t_*rt_skew*J_com_prev_.block(3,0,3,n));
    Eigen::MatrixXd Zq = (2*total_m_/delta_t_)*Sf_*J_com_prev_.block(3,0,3,n) ;
    Eigen::MatrixXd Zdq = delta_t_*St_*AM_prev_;

    Eigen::MatrixXd Zq_n = A_prev_.inverse()*J_ext_prev_.transpose()*Zq;
    Eigen::MatrixXd Zdq_n = A_prev_.inverse()*J_ext_prev_.transpose()*Zdq;
...
void AWBC::AdaptGains(const std::vector<ContactSpec*> & contact_list, Eigen::VectorXd & Kp_a, Eigen::VectorXd & Kd_a)
{
    ...
    for(int i(0); i< Kp_.size(); ++i){
        epsilon_p[i] = hZq_[6+i] + Fs[i]*Fs[i] - hat_Kp_[i];
        epsilon_d[i] = 2*Fs[i] + hZdq_[6+i] - hat_Kd_[i];
    }
