...
// Constructor
WBC::WBC(const std::vector<bool>& act_list)
    : num_act_joint_(0),
      num_passive_(0),
      dim_contacts_(0),
  ...
  myUtils::pretty_constructor(3, "WBC");
...
void WBC::getQddotResult(Eigen::VectorXd& qddot_out) {
  qddot_out = qddot_result_;
}
// Returns the reaction forces computed by the QP
void WBC::getFrResult(Eigen::VectorXd& Fr_out) { Fr_out = Fr_result_; }

void WBC::setQPWeights(const Eigen::VectorXd& w_task_hierarchy_in,
...
void WBC::setRegularizationTerms(const double lambda_qddot_in,
                                 const double lambda_Fr_in) {
  lambda_qddot = lambda_qddot_in;
  lambda_Fr = lambda_Fr_in;
}

void WBC::setTargetWrenchMinimization(
    const bool target_wrench_minimization_in) {
  target_wrench_minimization = target_wrench_minimization_in;
}

void WBC::setTorqueLimits(const Eigen::VectorXd& tau_min_in,
                          const Eigen::VectorXd& tau_max_in) {
  tau_min_ = tau_min_in;
  tau_max_ = tau_max_in;
}
void WBC::enableTorqueLimits(const bool b_torque_limit_in) {
  b_torque_limits_ = b_torque_limit_in;
}

void WBC::updateSetting(const Eigen::MatrixXd& A, const Eigen::MatrixXd& Ainv,
...
void WBC::solve(const std::vector<Task*>& task_list,
  ...
  if (!b_updatesetting_) {
    printf(
        "[Warning] WBC setting A, Ainv, cori, and grav terms have not been "
        "done\n");
  }
  ...
  // sum_i=1^n w_i|| J_i*qddot + J_i*dotqdot - x_iddot ||^2_2 +
  // lambda_qddot*||qddot||^2_2
  Task* task;
  for (int i = 0; i < task_list.size(); i++) {
    task = task_list[i];
    ...
    // std::cout << "task " << i << " xddot : " << xddot.transpose() <<
    // std::endl;
...
void WBC::buildContactStacks(const std::vector<ContactSpec*>& contact_list,
                             const Eigen::VectorXd& w_rf_contacts_in) {
...
void WBC::prepareQPSizes() {
  n_quadprog_ = dim_dec_vars_;  // Number of decision Variables
  p_quadprog_ =
      dim_inequality_constraints_;  // Number of Inequality constraints
...
void WBC::setQuadProgCosts(const Eigen::MatrixXd& P_cost,
...
void WBC::setEqualityConstraints(const Eigen::MatrixXd& Eq_mat,
...
void WBC::setInequalityConstraints(const Eigen::MatrixXd& IEq_mat,
