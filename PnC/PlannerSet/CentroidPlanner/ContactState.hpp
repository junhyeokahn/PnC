...
public:
    ...
    std::string toString() const {
        ...
        for (int eff_id=0; eff_id<CentroidModel::numEEf; eff_id++) {
            text << "eff_id " << eff_id << "\n";
            for (int cnt_id=0; cnt_id<eEfContacts[eff_id].size(); cnt_id++) {
                text << "  cnt_id " << cnt_id << "\n";
    ...
    friend std::ostream& operator<<(std::ostream &os, const ContactStateSequence& obj) { return os << obj.toString(); }
