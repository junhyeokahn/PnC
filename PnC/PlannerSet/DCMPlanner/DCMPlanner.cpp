...
void DCMPlanner::setRobotMass(double mass){
  robot_mass = mass;
}

void DCMPlanner::setInitialTime(double t_start_in){
    t_start = t_start_in;
}

double DCMPlanner::getInitialTime(){
  return t_start;
}
...
void DCMPlanner::initialize_footsteps_rvrp(const std::vector<Footstep> & input_footstep_list, 
...
void DCMPlanner::initialize_footsteps_rvrp(const std::vector<Footstep> & input_footstep_list, const Footstep & initial_footstance, const Eigen::Vector3d & initial_rvrp){
  if (input_footstep_list.size() == 0){ 
    return;
  }
...
void DCMPlanner::initialize_footsteps_rvrp(const std::vector<Footstep> & input_footstep_list, const Footstep & left_footstance, const Footstep & right_footstance){
  if (input_footstep_list.size() == 0){ 
    return;
  }
...
void DCMPlanner::initialize_footsteps_rvrp(const std::vector<Footstep> & input_footstep_list, const Footstep & left_footstance, const Footstep & right_footstance, const Eigen::Vector3d & initial_com){
  if (input_footstep_list.size() == 0){ 
    return;
  }
...
void DCMPlanner::get_average_rvrp(const Footstep & footstance_1, const Footstep & footstance_2, Eigen::Vector3d & average_rvrp){
...
Eigen::Vector3d DCMPlanner::computeDCM_ini_i(const Eigen::Vector3d & r_vrp_d_i, const double & t_step, const Eigen::Vector3d & dcm_eos_i){
...
double DCMPlanner::get_iniDS_transition_time(){
...
Eigen::Vector3d DCMPlanner::computeDCM_iniDS_i(const int & step_index, const double t_DS_ini){
  if (step_index == 0){
    return ini_dcm_pos;
  }
...
Eigen::Vector3d DCMPlanner::computeDCM_eoDS_i(const int & step_index, const double t_DS_end){
...
Eigen::Vector3d DCMPlanner::computeDCMvel_iniDS_i(const int & step_index, const double t_DS_ini){
  if (step_index == 0){
    return ini_dcm_vel;
  }
...
Eigen::Vector3d DCMPlanner::computeDCMvel_eoDS_i(const int & step_index, const double t_DS_end){
...
Eigen::Vector3d DCMPlanner::computeDCMacc_iniDS_i(const int & step_index, const double t_DS_ini){
  if (step_index == 0){
    return Eigen::Vector3d::Zero();
  }  
...
Eigen::Vector3d DCMPlanner::computeDCMacc_eoDS_i(const int & step_index, const double t_DS_end){
...
Eigen::MatrixXd DCMPlanner::polynomialMatrix(const double Ts,
...
Eigen::Vector3d DCMPlanner::get_DCM_exp(const int & step_index, const double & t){
  // Get t_step
...
Eigen::Vector3d DCMPlanner::get_DCM_vel_exp(const int & step_index, const double & t){
  // Get t_step
  double t_step = get_t_step(step_index);
  // Clamp time value
  double time = clampDOUBLE(t, 0.0, t_step);
...
Eigen::Vector3d DCMPlanner::get_DCM_acc_exp(const int & step_index, const double & t){
  // Get t_step
  double t_step = get_t_step(step_index);
  // Clamp time value
  double time = clampDOUBLE(t, 0.0, t_step);
...
Eigen::Vector3d DCMPlanner::get_DCM_DS_poly(const int & step_index, const double & t){
...
// time, t, is clamped between 0.0 and t_step.
Eigen::Vector3d DCMPlanner::get_DCM_DS_vel_poly(const int & step_index, const double & t){
  double Ts = get_polynomial_duration(step_index);
  double time = clampDOUBLE(t, 0.0, Ts);
...
// time, t, is clamped between 0.0 and t_step.
Eigen::Vector3d DCMPlanner::get_DCM_DS_minjerk(const int & step_index, const double & t){
  double Ts = get_polynomial_duration(step_index);
  double time = clampDOUBLE(t, 0.0, Ts);  
...
// time, t, is clamped between 0.0 and t_step.
Eigen::Vector3d DCMPlanner::get_DCM_DS_vel_minjerk(const int & step_index, const double & t){
  double Ts = get_polynomial_duration(step_index);
  double time = clampDOUBLE(t, 0.0, Ts);  
...
void DCMPlanner::get_ref_reaction_force(const double t, Eigen::Vector3d & f_out){
...
double DCMPlanner::get_total_trajectory_time(){
  return t_end;
}
...
bool DCMPlanner::get_t_swing_start_end(const int step_index, double & swing_start_time, double & swing_end_time){
...
double DCMPlanner::clampDOUBLE(double input, double low_bound, double upper_bound){
  if (input <= low_bound){
    return low_bound;
  }else if (input >= upper_bound){
    return upper_bound;
...
void DCMPlanner::get_com_vel(const Eigen::Vector3d & com_pos, const Eigen::Vector3d & dcm, Eigen::Vector3d & com_vel_out){
  com_vel_out = (-1.0/b)*(com_pos - dcm);
}

void DCMPlanner::get_reaction_force(const double mass, const Eigen::Vector3d & com_pos, 
...
void DCMPlanner::get_r_vrp(const double b_in, const Eigen::Vector3d & dcm, const Eigen::Vector3d & dcm_vel, Eigen::Vector3d & r_vrp_out){
...
void DCMPlanner::get_ref_ori_ang_vel_acc(const double t, Eigen::Quaterniond & quat_out,
