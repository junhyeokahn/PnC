...
SelectedJointTask2::SelectedJointTask2(RobotSystem* _robot,
...
SelectedJointTask2::~SelectedJointTask2() {}

bool SelectedJointTask2::_UpdateCommand(const Eigen::VectorXd& _pos_des,
...
bool SelectedJointTask2::_UpdateTaskJacobian() {
    for (int i(0); i < selected_jidx_.size(); ++i) {
        Jt_(i, selected_jidx_[i]) = 1.;
    }
    return true;
...
bool SelectedJointTask2::_UpdateTaskJDotQdot() {
    JtDotQdot_ = Eigen::VectorXd::Zero(dim_task_);
    return true;
}
