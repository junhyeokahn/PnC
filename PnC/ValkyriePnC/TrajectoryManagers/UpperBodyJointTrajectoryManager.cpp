...
UpperBodyJointTrajectoryManager::UpperBodyJointTrajectoryManager(Task* _upper_body_task, RobotSystem* _robot): TrajectoryManagerBase(_robot){
...
void UpperBodyJointTrajectoryManager::initializeRaiseRightArmNow(){
  std::cout << "[UpperBodyJointTrajectoryManager] intiialize raising arm" << std::endl;
...
void UpperBodyJointTrajectoryManager::initializeLowerRightArmNow(){
  std::cout << "[UpperBodyJointTrajectoryManager] intiialize lower arm" << std::endl;
  // get initial time and current position
  t_start_ = sp_->curr_time;
  pos_ini_ = sp_->q.tail(number_of_joints_);
  vel_ini_ = sp_->qdot.tail(number_of_joints_);
...
void UpperBodyJointTrajectoryManager::computeArmTrajectories(){
  ...
  if (raising_arm || lowering_arm){
    ...
    if (t_local <= arm_trajectory_duration_){
      pos_des_[rarm_joint_index_] = myUtils::smooth_changing(pos_ini_[rarm_joint_index_], target, arm_trajectory_duration_, t_local);
      vel_des_[rarm_joint_index_] = myUtils::smooth_changing_vel(vel_ini_[rarm_joint_index_], 0.0, arm_trajectory_duration_, t_local);
...
void UpperBodyJointTrajectoryManager::paramInitialization(const YAML::Node& node){ 
    try {
