...
void DoubleSupportStand::firstVisit() {
  ...
  Eigen::Vector3d lfoot_pos =
      robot_->getBodyNodeCoMIsometry(ValkyrieBodyNode::leftCOP_Frame)
          .translation();
  Eigen::Vector3d rfoot_pos =
      robot_->getBodyNodeCoMIsometry(ValkyrieBodyNode::rightCOP_Frame)
          .translation();
  des_com_pos_ = 0.5 * (lfoot_pos + rfoot_pos) + com_pos_target_;

  _SetBspline(ini_com_pos_, des_com_pos_);

  ini_pelvis_quat_ = Eigen::Quaternion<double>(
  ...
  des_pelvis_quat << ini_pelvis_quat_.w(), ini_pelvis_quat_.x(),
      ini_pelvis_quat_.y(), ini_pelvis_quat_.z();
  taf_container_->pelvis_ori_task_->updateDesired(
      des_pelvis_quat, Eigen::VectorXd::Zero(3), Eigen::VectorXd::Zero(3));
