...
class DynamicsState
{
    public:
        ...
        typedef std::array<Eigen::Quaternion<double>, CentroidModel::numEEf> OriArray;

        DynamicsState () : time(0.),
                           com(Eigen::VectorXd::Zero(3)),
                           aMom(Eigen::VectorXd::Zero(3)),
                           lMom(Eigen::VectorXd::Zero(3)),
...
class DynamicsStateSequence
{
    public:
        ...
        friend std::ostream& operator<<(std::ostream &os, const DynamicsStateSequence& obj) { return os << obj.toString(); }
