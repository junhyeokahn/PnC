...
#include <PnC/DracoPnC/DracoStateEstimator/IMUFrameEstimator.hpp>
...
IMUFrameEstimator::IMUFrameEstimator(RobotSystem* robot) : filtered_acc_(3) {
  ...
  for (int i(0); i < 3; ++i) {
    filtered_acc_[i] =
        new digital_lp_filter(cutoff_freq_, DracoAux::servo_rate);
  }
}
...
void IMUFrameEstimator::estimatorInitialization(const Eigen::VectorXd& acc,
  ...
  world_root_pris_vel_ =
      world_root_pris_vel_ + DracoAux::servo_rate * world_non_grav_accel_;
...
void IMUFrameEstimator::setSensorData(const Eigen::VectorXd& _acc,
  ...
  imuQworld__w.w() =
      imuQworld_.w() + -0.5 * imuQworld__w_dot.w() * DracoAux::servo_rate;
  imuQworld__w.x() =
      imuQworld_.x() + -0.5 * imuQworld__w_dot.x() * DracoAux::servo_rate;
  imuQworld__w.y() =
      imuQworld_.y() + -0.5 * imuQworld__w_dot.y() * DracoAux::servo_rate;
  imuQworld__w.z() =
      imuQworld_.z() + -0.5 * imuQworld__w_dot.z() * DracoAux::servo_rate;
  ...
  // DracoAux::servo_rate; world__est_w_dot.x() = (imuQworld_prev_.x() -
  // imuQworld_.x())
  // * 2.0 / DracoAux::servo_rate; world__est_w_dot.y() = (imuQworld_prev_.y()
  // - imuQworld_.y()) * 2.0 / DracoAux::servo_rate; world__est_w_dot.z() =
  // (imuQworld_prev_.z() - imuQworld_.z()) * 2.0 / DracoAux::servo_rate;
  ...
  // m_world__root_pris_vel*DracoAux::servo_rate +
  // 0.5*world_non_grav_accel_*DracoAux::servo_rate*DracoAux::servo_rate;
  // TODO : Check position
}
