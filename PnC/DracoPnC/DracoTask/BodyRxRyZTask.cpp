...
bool BodyRxRyZTask::_UpdateCommand(const Eigen::VectorXd& _pos_des,
  ...
  pos_err[2] = _pos_des[6] - com_pos[2];
  vel_des[2] = _vel_des[5];
  acc_des[2] = _acc_des[5];

  Eigen::VectorXd vel_act = Eigen::VectorXd::Zero(dim_task_);
  vel_act.head(2) =
      robot_->getBodyNodeCoMSpatialVelocity(DracoBodyNode::Torso).head(2);
  vel_act.tail(1) =
      robot_->getBodyNodeCoMSpatialVelocity(DracoBodyNode::Torso).tail(1);
