...
BodyTest::BodyTest(RobotSystem* robot) : Test(robot) {
    ...
    cfg_ = YAML::LoadFile(THIS_COM"Config/Draco/TEST/BODY_TEST.yaml");
...
void BodyTest::TestInitialization() {
    ...
    body_ctrl_->ctrlInitialization(cfg_["control_configuration"]["body_ctrl"]);
}
...
void BodyTest::_SettingParameter() {
    try {
        ...
        myUtils::readParameter(test_cfg, "body_ctrl_time", tmp_val);
