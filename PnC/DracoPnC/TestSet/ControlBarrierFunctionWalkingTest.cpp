...
ControlBarrierFunctionWalkingTest::ControlBarrierFunctionWalkingTest(
    RobotSystem* robot)
    : Test(robot) {
    myUtils::pretty_constructor(1, "CBF Walking Test");
    cfg_ = YAML::LoadFile(THIS_COM "Config/Draco/TEST/CBF_WALKING_TEST.yaml");
...
void ControlBarrierFunctionWalkingTest::TestInitialization() {
...
int ControlBarrierFunctionWalkingTest::_NextPhase(const int& phase) {
    ...
    if (phase == CBFWkPhase::double_contact_2) {
        ++num_step_;
        printf("%i th step : ", num_step_);
    ...
    // !! TEST !!
    // if (((phase == CBFWkPhase::double_contact_1) ||
    //(phase == CBFWkPhase::double_contact_2)) &&
    //(num_step_ > 1)) {
    ...
    if (next_phase == CBFWkPhase::NUM_WALKING_PHASE) {
        return CBFWkPhase::double_contact_1;
    } else {
        return next_phase;
    }
}

void ControlBarrierFunctionWalkingTest::_SettingParameter() {
    try {
        double tmp;
        bool b_tmp;
