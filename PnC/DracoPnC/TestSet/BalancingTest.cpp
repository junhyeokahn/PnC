...
void BalancingTest::TestInitialization() {
    ...
    if (DYNAMIC_BALANCING) {
        balancing_ctrl_->ctrlInitialization(cfg_["control_configuration"]["balancing_ctrl"]);
    } else {
        kin_balancing_ctrl_->ctrlInitialization(cfg_["control_configuration"]["kin_balancing_ctrl"]);
    }
}
...
void BalancingTest::_SettingParameter() {
    try {
        ...
        myUtils::readParameter(test_cfg, "balancing_ctrl_time", tmp_val);
        if (DYNAMIC_BALANCING) {
            ((BalancingCtrl*)balancing_ctrl_)->setBalancingTime(tmp_val);
        } else {
            ((KinBalancingCtrl*)kin_balancing_ctrl_)->setBalancingTime(tmp_val);
        }
        myUtils::readParameter(test_cfg, "interpolation_time", tmp_val);
        if (DYNAMIC_BALANCING) {
