...
DracoStateEstimator::DracoStateEstimator(RobotSystem* robot) {
    ...
    global_euler_zyx_.setZero();
    global_euler_zyx_dot_.setZero();
    ...
    x_vel_est_ = new AverageFilter(SERVO_RATE, 0.01, 1.5);
...
void DracoStateEstimator::initialization(DracoSensorData* data){
    ...
    ori_est_->getEstimatedState(global_euler_zyx_, global_euler_zyx_dot_);
...
void DracoStateEstimator::update(DracoSensorData* data){
    ...
    if ((sp_->phase_copy == 2) && (!visit_once)){
        ((AverageFilter*)x_vel_est_)->initialization(sp_->com_vel[0]);
        ((AverageFilter*)y_vel_est_)->initialization(sp_->com_vel[1]);
        //mocap_x_vel_est_->Initialization(sp_->CoM_vel_[0], sp_->CoM_vel_[1]);
        //mocap_y_vel_est_->Initialization(sp_->CoM_vel_[0], sp_->CoM_vel_[1]);
        visit_once = true;
    }
...
void DracoStateEstimator::_ConfigurationAndModelUpdate() {
    ...
    curr_config_[3] = global_euler_zyx_[0];
    curr_config_[4] = global_euler_zyx_[1];
    curr_config_[5] = global_euler_zyx_[2];
