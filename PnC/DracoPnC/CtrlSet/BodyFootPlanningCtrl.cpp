...
/*    Eigen::VectorXd yaw_pos = Eigen::VectorXd::Zero(2);*/
    ...
    if (state_machine_time_ < end_time_/2.0) {
        curr_ankle_pos_des = myUtils::smooth_changing(ini_ankle_, fin_ankle_, end_time_/2.0, state_machine_time_);
        curr_ankle_vel_des = myUtils::smooth_changing_vel(ini_ankle_, fin_ankle_, end_time_/2.0, state_machine_time_);
        curr_ankle_acc_des = myUtils::smooth_changing_acc(ini_ankle_, fin_ankle_, end_time_/2.0, state_machine_time_);
    } else {
    ...
    foot_point_task_->updateTask(point_foot_pos_des, point_foot_vel_des, point_foot_acc_des);
...
bool BodyFootPlanningCtrl::endOfPhase(){
    ...
    if (contact_check_with_ankle) {
        ...
        if(state_machine_time_ > end_time_ * 0.5 && contact_happen_by_ankle){
            printf("[Config Body Foot Ctrl] contact happen by ankle, state_machine_time/ end time: (%f, %f)\n",
                    state_machine_time_, end_time_);
            return true;
        }
    }
