...
void JPosTargetCtrl::_jpos_task_setup(){
    ...
    for(int i(0); i<robot_->getNumActuatedDofs(); ++i){
        des_jpos_[i] = myUtils::smooth_changing(jpos_ini_[i], jpos_target_[i], end_time_, state_machine_time_);
        des_jvel_[i] = myUtils::smooth_changing_vel(jpos_ini_[i], jpos_target_[i], end_time_, state_machine_time_);
        jacc_des[i] = myUtils::smooth_changing_acc(jpos_ini_[i], jpos_target_[i], end_time_, state_machine_time_);
    }
