...
double MPCWalkCtrl::clamp_value(double in, double min, double max){
    if (in >= max){
        return max;
    }else if (in <= min){
        return min;
...
double MPCWalkCtrl::computeAlphaGivenBreakFrequency(double hz, double dt){
...
void MPCWalkCtrl::oneStep(void* _cmd) {
...
void MPCWalkCtrl::_mpc_Xdes_setup(){
...
void MPCWalkCtrl::lastVisit() {}

bool MPCWalkCtrl::endOfPhase() {
...
void MPCWalkCtrl::ctrlInitialization(const YAML::Node& node) {
    jpos_ini_ = sp_->q.segment(robot_->getNumVirtualDofs(),
                               robot_->getNumActuatedDofs());
