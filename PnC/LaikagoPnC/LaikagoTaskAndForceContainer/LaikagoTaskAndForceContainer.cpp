...
LaikagoTaskAndForceContainer::LaikagoTaskAndForceContainer(RobotSystem* _robot)
    : TaskAndForceContainer(_robot) {
  _InitializeTasks();
  _InitializeContacts();
}

LaikagoTaskAndForceContainer::~LaikagoTaskAndForceContainer() {
  _DeleteTasks();
  _DeleteContacts();
}

void LaikagoTaskAndForceContainer::_InitializeTasks() {
  myUtils::pretty_constructor(2, "Laikago Task And Force Container");
  ...
  base_ori_task_ =
      new BasicTask(robot_, BasicTaskType::LINKORI, 3, LaikagoBodyNode::trunk);

  // Set Foot Motion Tasks
  rfoot_center_pos_task_ = new BasicTask(
      robot_, BasicTaskType::ISOLATED_LINKXYZ, 3, LaikagoBodyNode::FR_foot);
  lfoot_center_pos_task_ = new BasicTask(
      robot_, BasicTaskType::ISOLATED_LINKXYZ, 3, LaikagoBodyNode::FL_foot);
  rfoot_center_ori_task_ =
      new FootLocalRyRzTask(robot_, LaikagoBodyNode::FR_foot);
  lfoot_center_ori_task_ =
      new FootLocalRyRzTask(robot_, LaikagoBodyNode::FL_foot);
...
void LaikagoTaskAndForceContainer::_InitializeContacts() {
  // rfoot_front_contact_ =
  // new PointContactSpec(robot_, LaikagoBodyNode::rFootFront, 0.7);
  // rfoot_back_contact_ =
  // new PointContactSpec(robot_, LaikagoBodyNode::rFootBack, 0.7);
  // lfoot_front_contact_ =
  // new PointContactSpec(robot_, LaikagoBodyNode::lFootFront, 0.7);
  // lfoot_back_contact_ =
  // new PointContactSpec(robot_, LaikagoBodyNode::lFootBack, 0.7);

  // dim_contact_ = rfoot_front_contact_->getDim() +
  // rfoot_back_contact_->getDim() +
  // lfoot_front_contact_->getDim() + lfoot_back_contact_->getDim();

  rfoot_contact_ = new SurfaceContactSpec(robot_, LaikagoBodyNode::FR_foot,
                                          0.04, 0.01, 0.9);
  lfoot_contact_ = new SurfaceContactSpec(robot_, LaikagoBodyNode::FL_foot,
                                          0.04, 0.01, 0.9);
...
void LaikagoTaskAndForceContainer::_DeleteTasks() {
  delete joint_task_;
  // delete dcm_task_;
  delete com_task_;
...
void LaikagoTaskAndForceContainer::_DeleteContacts() {
...
void LaikagoTaskAndForceContainer::paramInitialization(const YAML::Node& node) {
  try {
    double temp_double;
