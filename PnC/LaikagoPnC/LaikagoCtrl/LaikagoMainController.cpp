...
LaikagoMainController::LaikagoMainController(
    LaikagoTaskAndForceContainer* _taf_container, RobotSystem* _robot) {
  myUtils::pretty_constructor(2, "Laikago Main Controller");
  // Initialize Flag
  b_first_visit_ = true;
  ...
  act_list.resize(Laikago::n_dof, true);
  for (int i(0); i < Laikago::n_vdof; ++i) act_list[i] = false;
  wbc_ = new WBC(act_list);

  tau_cmd_ = Eigen::VectorXd::Zero(Laikago::n_adof);
  qddot_cmd_ = Eigen::VectorXd::Zero(Laikago::n_adof);

  // Initialize Joint Integrator
  wbc_dt_ = LaikagoAux::servo_rate;
  joint_integrator_ = new JointIntegrator(Laikago::n_adof, wbc_dt_);

  // Initialize desired pos, vel, acc containers
  des_jpos_ = Eigen::VectorXd::Zero(Laikago::n_adof);
  des_jvel_ = Eigen::VectorXd::Zero(Laikago::n_adof);
  des_jacc_ = Eigen::VectorXd::Zero(Laikago::n_adof);
}
...
void LaikagoMainController::_PreProcessing_Command() {
  // Update Dynamic Terms
  A_ = robot_->getMassMatrix();
  A_rotor_ = A_;
  for (int i = 0; i < Laikago::n_adof; ++i) {
    A_rotor_(i + Laikago::n_vdof, i + Laikago::n_vdof) += sp_->rotor_inertia[i];
  }
...
void LaikagoMainController::getCommand(void* _cmd) {
  ...
  if (joint_integrator_->isInitialized()) {
    joint_integrator_->integrate(
        des_jacc_, sp_->qdot.segment(Laikago::n_vdof, Laikago::n_adof),
        sp_->q.segment(Laikago::n_vdof, Laikago::n_adof), des_jvel_, des_jpos_);
  } else {
    des_jpos_ = sp_->q.segment(Laikago::n_vdof, Laikago::n_adof);
    des_jvel_ = sp_->qdot.segment(Laikago::n_vdof, Laikago::n_adof);
  }
  ...
  // Set Command
  for (int i(0); i < Laikago::n_adof; ++i) {
    ((LaikagoCommand*)_cmd)->jtrq[i] = tau_cmd_[i];
    ((LaikagoCommand*)_cmd)->q[i] = des_jpos_[i];
    ((LaikagoCommand*)_cmd)->qdot[i] = des_jvel_[i];
  }

  // myUtils::pretty_print(((LaikagoCommand*)_cmd)->jtrq, std::cout, "jtrq");
  // myUtils::pretty_print(((LaikagoCommand*)_cmd)->q, std::cout, "q");
  // myUtils::pretty_print(((LaikagoCommand*)_cmd)->qdot, std::cout, "qdot");
}

void LaikagoMainController::initializeJointIntegrator() {
  Eigen::VectorXd jpos_ini = sp_->q.segment(Laikago::n_vdof, Laikago::n_adof);
  joint_integrator_->initializeStates(Eigen::VectorXd::Zero(Laikago::n_adof),
                                      jpos_ini);
}

void LaikagoMainController::firstVisit() { initializeJointIntegrator(); }

void LaikagoMainController::ctrlInitialization(const YAML::Node& node) {
  // WBC Defaults
  wbc_dt_ = LaikagoAux::servo_rate;
  ...
  Eigen::VectorXd tau_min =
      robot_->GetTorqueLowerLimits().segment(Laikago::n_vdof, Laikago::n_adof);
  Eigen::VectorXd tau_max =
      robot_->GetTorqueUpperLimits().segment(Laikago::n_vdof, Laikago::n_adof);
  ...
  joint_integrator_->setVelocityBounds(
      robot_->getVelocityLowerLimits().segment(Laikago::n_vdof, Laikago::n_adof),
      robot_->getVelocityUpperLimits().segment(Laikago::n_vdof, Laikago::n_adof));
  joint_integrator_->setPositionBounds(
      robot_->getPositionLowerLimits().segment(Laikago::n_vdof, Laikago::n_adof),
      robot_->getPositionUpperLimits().segment(Laikago::n_vdof, Laikago::n_adof));
}
