...
LaikagoStateProvider* LaikagoStateProvider::getStateProvider(RobotSystem* _robot) {
...
LaikagoStateProvider::LaikagoStateProvider(RobotSystem* _robot) {
  ...
  robot_ = _robot;
  stance_foot = LaikagoBodyNode::FL_foot;
...
void LaikagoStateProvider::saveCurrentData() {
  ...
  rfoot_center_so3 =
      robot_->getBodyNodeSpatialVelocity(LaikagoBodyNode::FR_foot).head(3);
  lfoot_center_so3 =
      robot_->getBodyNodeSpatialVelocity(LaikagoBodyNode::FL_foot).head(3);

  base_quat = Eigen::Quaternion<double>(
      robot_->getBodyNodeIsometry(LaikagoBodyNode::trunk).linear());
  base_ang_vel =
      robot_->getBodyNodeCoMSpatialVelocity(LaikagoBodyNode::trunk).head(3);
}
