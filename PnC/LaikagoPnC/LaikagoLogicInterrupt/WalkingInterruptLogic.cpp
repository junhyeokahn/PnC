...
WalkingInterruptLogic::WalkingInterruptLogic(
    ...
    : InterruptLogic() {
  myUtils::pretty_constructor(1, "Laikago Walking Interrupt Logic");
  ctrl_arch_ = _ctrl_arch;
  sp_ = LaikagoStateProvider::getStateProvider(ctrl_arch_->robot_);
  swaying_dis_ = 0.05;
}
...
void WalkingInterruptLogic::processInterrupts() {
  if (b_interrupt_button_p) {
    ...
    if (ctrl_arch_->getState() == Laikago_STATES::BALANCE) {
      double amp(0.03);
      double freq(0.5);
  ...
  if (b_interrupt_button_w) {
    ...
    if (ctrl_arch_->getState() == Laikago_STATES::BALANCE) {
      ...
      static_cast<DoubleSupportBalance*>(
          ctrl_arch_->state_machines_[Laikago_STATES::BALANCE])
