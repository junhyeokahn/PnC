...
LaikagoControlArchitecture::LaikagoControlArchitecture(RobotSystem* _robot)
    : ControlArchitecture(_robot) {
  b_state_first_visit_ = true;

  myUtils::pretty_constructor(1, "Laikago Control Architecture");
  cfg_ = YAML::LoadFile(THIS_COM "Config/Laikago/TEST/WALKING_PARAMS.yaml");

  sp_ = LaikagoStateProvider::getStateProvider(robot_);

  // Initialize Main Controller
  taf_container_ = new LaikagoTaskAndForceContainer(robot_);
  taf_container_->paramInitialization(cfg_["task_parameters"]);
  main_controller_ = new LaikagoMainController(taf_container_, robot_);
  ...
  dcm_trajectory_manager_ = new DCMTrajectoryManager(
      ...
      robot_, LaikagoBodyNode::FL_foot, LaikagoBodyNode::FR_foot);

  // Initialize states: add all states to the state machine map
  state_machines_[Laikago_STATES::INITIALIZE] =
      new Initialize(Laikago_STATES::INITIALIZE, this, robot_);
  state_machines_[Laikago_STATES::STAND] =
      new DoubleSupportStand(Laikago_STATES::STAND, this, robot_);
  state_machines_[Laikago_STATES::BALANCE] =
      new DoubleSupportBalance(Laikago_STATES::BALANCE, this, robot_);

  state_machines_[Laikago_STATES::RL_CONTACT_TRANSITION_START] =
      new ContactTransitionStart(Laikago_STATES::RL_CONTACT_TRANSITION_START,
                                 RIGHT_ROBOT_SIDE, this, robot_);
  state_machines_[Laikago_STATES::RL_CONTACT_TRANSITION_END] =
      new ContactTransitionEnd(Laikago_STATES::RL_CONTACT_TRANSITION_END,
                               RIGHT_ROBOT_SIDE, this, robot_);
  state_machines_[Laikago_STATES::RL_SWING] =
      new SwingControl(Laikago_STATES::RL_SWING, RIGHT_ROBOT_SIDE, this, robot_);

  state_machines_[Laikago_STATES::LL_CONTACT_TRANSITION_START] =
      new ContactTransitionStart(Laikago_STATES::LL_CONTACT_TRANSITION_START,
                                 LEFT_ROBOT_SIDE, this, robot_);
  state_machines_[Laikago_STATES::LL_CONTACT_TRANSITION_END] =
      new ContactTransitionEnd(Laikago_STATES::LL_CONTACT_TRANSITION_END,
                               LEFT_ROBOT_SIDE, this, robot_);
  state_machines_[Laikago_STATES::LL_SWING] =
      new SwingControl(Laikago_STATES::LL_SWING, LEFT_ROBOT_SIDE, this, robot_);

  // Set Starting State
  state_ = Laikago_STATES::INITIALIZE;
  prev_state_ = state_;

  _InitializeParameters();
}

LaikagoControlArchitecture::~LaikagoControlArchitecture() {
  ...
  delete state_machines_[Laikago_STATES::INITIALIZE];
  delete state_machines_[Laikago_STATES::STAND];
  delete state_machines_[Laikago_STATES::BALANCE];
  delete state_machines_[Laikago_STATES::RL_CONTACT_TRANSITION_START];
  delete state_machines_[Laikago_STATES::RL_CONTACT_TRANSITION_END];
  delete state_machines_[Laikago_STATES::RL_SWING];
  delete state_machines_[Laikago_STATES::LL_CONTACT_TRANSITION_START];
  delete state_machines_[Laikago_STATES::LL_CONTACT_TRANSITION_END];
  delete state_machines_[Laikago_STATES::LL_SWING];
}

void LaikagoControlArchitecture::ControlArchitectureInitialization() {}

void LaikagoControlArchitecture::getCommand(void* _command) {
  // Initialize State
  ...
  // if ((prev_state_ == Laikago_STATES::STAND || state_ == Laikago_STATES::BALANCE)
  // &&
  // b_integrator_init) {
  ...
  if (state_ == Laikago_STATES::INITIALIZE) {
    getIVDCommand(_command);
  } else {
    if (state_ == Laikago_STATES::BALANCE &&
  ...
  // TEST : Foot damping
  if (state_ != Laikago_STATES::INITIALIZE) {
    //((LaikagoCommand*)_command)->qdot[4] = 0.;
    //((LaikagoCommand*)_command)->qdot[9] = 0.;
    //((LaikagoCommand*)_command)->qdot = Eigen::VectorXd::Zero(10);
  }
  // TEST END
};

void LaikagoControlArchitecture::smoothing_torque(void* _cmd) {
  if (state_ == Laikago_STATES::INITIALIZE) {
    double rat = ((Initialize*)state_machines_[state_])->progression_variable();
    for (int i = 0; i < Laikago::n_adof; ++i) {
      ((LaikagoCommand*)_cmd)->jtrq[i] =
          myUtils::smoothing(0, ((LaikagoCommand*)_cmd)->jtrq[i], rat);
      sp_->prev_trq_cmd[i] = ((LaikagoCommand*)_cmd)->jtrq[i];
    }
  }
  if (state_ == Laikago_STATES::STAND) {
    double rat =
        ((DoubleSupportStand*)state_machines_[state_])->progression_variable();
    for (int i = 0; i < Laikago::n_adof; ++i) {
      ((LaikagoCommand*)_cmd)->jtrq[i] = myUtils::smoothing(
          sp_->prev_trq_cmd[i], ((LaikagoCommand*)_cmd)->jtrq[i], rat);
    }
  }
}

void LaikagoControlArchitecture::getIVDCommand(void* _cmd) {
  Eigen::VectorXd tau_cmd = Eigen::VectorXd::Zero(Laikago::n_adof);
  Eigen::VectorXd des_jpos = Eigen::VectorXd::Zero(Laikago::n_adof);
  Eigen::VectorXd des_jvel = Eigen::VectorXd::Zero(Laikago::n_adof);

  Eigen::MatrixXd A = robot_->getMassMatrix();
  Eigen::VectorXd grav = robot_->getGravity();
  Eigen::VectorXd cori = robot_->getCoriolis();

  Eigen::VectorXd xddot_des = Eigen::VectorXd::Zero(Laikago::n_adof);
  ...
  Eigen::VectorXd qddot_des = Eigen::VectorXd::Zero(Laikago::n_dof);
  qddot_des.tail(Laikago::n_adof) = xddot_des;

  des_jpos = sp_->q.tail(Laikago::n_adof);
  des_jvel = sp_->qdot.tail(Laikago::n_adof);
  tau_cmd = (A * qddot_des + cori + grav).tail(Laikago::n_adof);

  for (int i(0); i < Laikago::n_adof; ++i) {
    ((LaikagoCommand*)_cmd)->jtrq[i] = tau_cmd[i];
    ((LaikagoCommand*)_cmd)->q[i] = des_jpos[i];
    ((LaikagoCommand*)_cmd)->qdot[i] = des_jvel[i];
  }
}

void LaikagoControlArchitecture::_InitializeParameters() {
  ...
  state_machines_[Laikago_STATES::INITIALIZE]->initialization(
      cfg_["state_initialize_params"]);
  state_machines_[Laikago_STATES::STAND]->initialization(
      cfg_["state_stand_params"]);
  state_machines_[Laikago_STATES::RL_SWING]->initialization(cfg_["state_swing"]);
  state_machines_[Laikago_STATES::LL_SWING]->initialization(cfg_["state_swing"]);
}

void LaikagoControlArchitecture::saveData() {
  ...
  sp_->q_task = sp_->q.tail(Laikago::n_adof);
  sp_->qdot_task = sp_->qdot.tail(Laikago::n_adof);

  if (state_ == Laikago_STATES::INITIALIZE || state_ == Laikago_STATES::STAND ||
      prev_state_ == Laikago_STATES::STAND) {
