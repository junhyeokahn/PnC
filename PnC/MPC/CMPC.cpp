...
// dpx,  dpy,  dpz,  g
void CMPC::setCostVec(const Eigen::VectorXd& cost_vec_in) {
    if (cost_vec_in.size() != 13) {
        printf(
            "Warning. Vector Cost is not of size 13. Will use old cost "
            "values:");
    } else {
...
Eigen::VectorXd CMPC::getx0(const double roll, const double pitch,
                            ...
                            const double com_y_rate, const double com_z_rate) {
...
void CMPC::convert_r_feet_to_com_frame(const Eigen::VectorXd& p_com,
...
void CMPC::assemble_vec_to_matrix(const int& n, const int& m,
...
void CMPC::integrate_robot_dynamics(const double& dt,
...
void CMPC::cont_time_state_space(const Eigen::VectorXd& x_current,
                                 const Eigen::MatrixXd& r_feet,
    ...
    if (rotate_inertia) {
        I_world = Rz * I_robot *
                  Rz.transpose();  // Inertia matrix aligned to CoM frame.
    } else {
        I_world = I_robot;
    }
...
void CMPC::discrete_time_state_space(const Eigen::MatrixXd& A,
...
void CMPC::qp_matrices(const Eigen::MatrixXd& Adt, const Eigen::MatrixXd& Bdt,
...
void CMPC::get_force_constraints(const int& n_Fr, Eigen::MatrixXd& CMat,
                                 Eigen::VectorXd& cvec) {
...
void CMPC::get_qp_constraints(const Eigen::MatrixXd& CMat,
...
void CMPC::solve_mpc(const Eigen::VectorXd& x0, const Eigen::VectorXd& X_des,
...
void CMPC::get_constant_desired_x(const Eigen::VectorXd& x_des,
