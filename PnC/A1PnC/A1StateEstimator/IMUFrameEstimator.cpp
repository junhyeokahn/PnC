...
#include <PnC/A1PnC/A1StateEstimator/IMUFrameEstimator.hpp>
...
void IMUFrameEstimator::estimatorInitialization(const Eigen::VectorXd& acc,
  ...
  imuTroot_ = robot_->getBodyNodeIsometry(A1BodyNode::imu_link);
  ...
  world_root_pris_vel_ =
      world_root_pris_vel_ + A1Aux::servo_rate * world_non_grav_accel_;
...
void IMUFrameEstimator::setSensorData(const Eigen::VectorXd& _acc,
  ...
  imuQworld__w.w() =
      imuQworld_.w() + -0.5 * imuQworld__w_dot.w() * A1Aux::servo_rate;
  imuQworld__w.x() =
      imuQworld_.x() + -0.5 * imuQworld__w_dot.x() * A1Aux::servo_rate;
  imuQworld__w.y() =
      imuQworld_.y() + -0.5 * imuQworld__w_dot.y() * A1Aux::servo_rate;
  imuQworld__w.z() =
      imuQworld_.z() + -0.5 * imuQworld__w_dot.z() * A1Aux::servo_rate;
