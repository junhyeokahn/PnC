...
void GaitData::zero() {
  ...
  periodTime = Eigen::Vector4d::Zero();           // overall gait period time
  timeStance = Eigen::Vector4d::Zero();           // total stance time
  timeSwing = Eigen::Vector4d::Zero();            // total swing time
  timeStanceRemaining = Eigen::Vector4d::Zero();  // stance time remaining
  timeSwingRemaining = Eigen::Vector4d::Zero();   // swing time remaining

  // Phase based descriptors
  switchingPhase = Eigen::Vector4d::Zero();  // phase to switch to swing
  phaseVariable = Eigen::Vector4d::Zero();   // overall gait phase for each foot
  phaseOffset = Eigen::Vector4d::Zero();     // nominal gait phase offsets
  phaseScale = Eigen::Vector4d::Zero();      // phase scale relative to variable
  phaseStance = Eigen::Vector4d::Zero();     // stance subphase
  phaseSwing = Eigen::Vector4d::Zero();      // swing subphase
 ...
 */
GaitScheduler::GaitScheduler(double _dt) {//MIT_UserParameters* _userParameters,
  initialize();
...
void GaitScheduler::modifyGait() {
  ...
  case 1:
    ...
    if (gaitData._currentGait != GaitType(gaitData.gait_type)) {
      gaitData._nextGait = GaitType(gaitData.gait_type);
      createGait();
    }
    break;
  ...
  case 2:
    ...
    if (fabs(gaitData.periodTimeNominal - gaitData.gait_period_time) > 0.0001 ||
        fabs(gaitData.switchingPhaseNominal - gaitData.gait_switching_phase) > 0.0001) {
      // Modify the gait with new parameters if it is overrideable
      if (gaitData.overrideable == 1) {
        gaitData.periodTimeNominal = gaitData.gait_period_time;
        gaitData.switchingPhaseNominal = gaitData.gait_switching_phase;
...
void GaitScheduler::createGait() {
  ...
  case GaitType::TRANSITION_TO_STAND:
    ...
    gaitData.gaitEnabled << 1, 1, 1, 1;
    double oldGaitPeriodTimeNominal = gaitData.periodTimeNominal;
...
void GaitScheduler::calcAuxiliaryGaitData() {
  ...
  if (gaitData.overrideable == 1) {
    ...
    } else if (gaitData.gait_override == 4) {
      //gaitData.periodTimeNominal = userParameters->gait_period_time;
      //gaitData.switchingPhaseNominal = userParameters->gait_switching_phase;
    }
  }
 ...
 */
void GaitScheduler::printGaitInfo() {
