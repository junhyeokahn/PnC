...
Eigen::Vector3d LocomotionSolution::get_des_com_pos(double t) {
  return spline_holder_.base_linear_->GetPoint(t).p();
}
Eigen::Vector3d LocomotionSolution::get_des_com_vel(double t) {
  return spline_holder_.base_linear_->GetPoint(t).v();
}
Eigen::Quaternion<double> LocomotionSolution::get_des_base_quat(double t) {
  Eigen::Vector3d ea = spline_holder_.base_angular_->GetPoint(t).p();
  Eigen::Quaternion<double> quat = myUtils::EulerZYXtoQuat(ea[0], ea[1], ea[2]);
  return quat;
}
Eigen::Vector3d LocomotionSolution::get_des_base_ang_vel(double t) {
  return spline_holder_.base_angular_->GetPoint(t).v();
}

Eigen::Vector3d LocomotionSolution::get_des_lf_pos(double t) {
  return spline_holder_.ee_motion_linear_.at(L)->GetPoint(t).p();
}
Eigen::Vector3d LocomotionSolution::get_des_lf_vel(double t) {
  return spline_holder_.ee_motion_linear_.at(L)->GetPoint(t).v();
}
Eigen::Quaternion<double> LocomotionSolution::get_des_lf_quat(double t) {
  Eigen::Vector3d ea = spline_holder_.ee_motion_angular_.at(L)->GetPoint(t).p();
  Eigen::Quaternion<double> quat = myUtils::EulerZYXtoQuat(ea[0], ea[1], ea[2]);
  return quat;
}
Eigen::Vector3d LocomotionSolution::get_des_lf_ang_vel(double t) {
  return spline_holder_.ee_motion_angular_.at(L)->GetPoint(t).v();
  ;
}
Eigen::Vector3d LocomotionSolution::get_des_rf_pos(double t) {
  return spline_holder_.ee_motion_linear_.at(R)->GetPoint(t).p();
}
Eigen::Vector3d LocomotionSolution::get_des_rf_vel(double t) {
  return spline_holder_.ee_motion_linear_.at(R)->GetPoint(t).v();
}
Eigen::Quaternion<double> LocomotionSolution::get_des_rf_quat(double t) {
  Eigen::Vector3d ea = spline_holder_.ee_motion_angular_.at(R)->GetPoint(t).p();
  Eigen::Quaternion<double> quat = myUtils::EulerZYXtoQuat(ea[0], ea[1], ea[2]);
  return quat;
}
Eigen::Vector3d LocomotionSolution::get_des_rf_ang_vel(double t) {
  return spline_holder_.ee_motion_angular_.at(R)->GetPoint(t).v();
}

Eigen::VectorXd LocomotionSolution::get_des_lf_wrench(double t) {
  Eigen::VectorXd ret = Eigen::VectorXd::Zero(6);
  ret.head(3) = spline_holder_.ee_wrench_angular_.at(L)->GetPoint(t).p();
  ret.tail(3) = spline_holder_.ee_wrench_linear_.at(L)->GetPoint(t).p();
  return ret;
}
Eigen::VectorXd LocomotionSolution::get_des_rf_wrench(double t) {
  Eigen::VectorXd ret = Eigen::VectorXd::Zero(6);
  ret.head(3) = spline_holder_.ee_wrench_angular_.at(R)->GetPoint(t).p();
  ret.tail(3) = spline_holder_.ee_wrench_linear_.at(R)->GetPoint(t).p();
  return ret;
}
