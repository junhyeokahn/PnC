...
moving_average_filter::moving_average_filter(int num_data)
...
double moving_average_filter::output() { return sum_ / num_data_; }
void moving_average_filter::clear(void) {
    sum_ = 0.0;
    memset((void *)buffer_, 0.0, sizeof(double) * num_data_);
}

moving_average_filter::~moving_average_filter() { delete[] buffer_; }

butterworth_filter::butterworth_filter(int num_sample, double dt,
...
butterworth_filter::~butterworth_filter(void) { delete[] mpBuffer; }
...
void butterworth_filter::input(double input_value) {
    ...
    for (j = 0; j < mNumSample; j++) {
        ...
        //		value += sqrt_2 * exp(-1./sqrt_2*t) * sin(1./sqrt_2*t )
        //* mDt;
    }
    mValue = value;
}

double butterworth_filter::output(void) { return mValue; }
...
double digital_lp_filter::output(void) { return lpf_out; }
...
double deriv_lp_filter::output(void) { return lpf_out; }
...
double ff01_filter::output(void) { return lpf_out; }
...
double ff02_filter::output(void) { return lpf_out; }
