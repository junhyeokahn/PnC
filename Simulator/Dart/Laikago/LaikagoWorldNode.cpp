...
LaikagoWorldNode::LaikagoWorldNode(const dart::simulation::WorldPtr& _world)
    : dart::gui::osg::WorldNode(_world),
      count_(0),
      t_(0.0),
      servo_rate_(0.001) {
  world_ = _world;
  skel_ = world_->getSkeleton("Laikago");
  ...
  interface_ = new LaikagoInterface();
  sensor_data_ = new LaikagoSensorData();
  command_ = new LaikagoCommand();

  resetButtonFlags();
  set_parameters_();
}

LaikagoWorldNode::~LaikagoWorldNode() {
  delete interface_;
  delete sensor_data_;
  delete command_;
}

void LaikagoWorldNode::set_parameters_() {
  try {
...
void LaikagoWorldNode::customPreStep() {
...
void LaikagoWorldNode::get_imu_data_(Eigen::VectorXd& ang_vel,
                                   Eigen::VectorXd& acc,
...
void LaikagoWorldNode::check_foot_contact_by_ft_(bool& rfoot_contact,
                                               bool& lfoot_contact) {
...
void LaikagoWorldNode::check_foot_contact_by_pos_(bool& rfoot_contact,
                                                bool& lfoot_contact) {
  Eigen::VectorXd f_r = skel_->getBodyNode("FR_foot")->getCOM();
  Eigen::VectorXd f_l = skel_->getBodyNode("FL_foot")->getCOM();
  Eigen::VectorXd r_r = skel_->getBodyNode("RR_foot")->getCOM();
  Eigen::VectorXd r_l = skel_->getBodyNode("RL_foot")->getCOM();
...
void LaikagoWorldNode::hold_rot_() {
...
void LaikagoWorldNode::hold_xy_() {
...
void LaikagoWorldNode::get_force_torque_data_() {
