...
#include <Simulator/Dart/Laikago/LaikagoWorldNode.hpp>
...
class OneStepProgress : public osgGA::GUIEventHandler {
 public:
  OneStepProgress(LaikagoWorldNode* worldnode) : worldnode_(worldnode) {}
...
void _setInitialConfiguration(dart::dynamics::SkeletonPtr robot) {
  int fr_hipIdx = robot->getDof("FR_hip_joint")->getIndexInSkeleton();
  int fr_thighIdx = robot->getDof("FR_thigh_joint")->getIndexInSkeleton();
  int fr_calfIdx  = robot->getDof("FR_calf_joint")->getIndexInSkeleton();
  //int fr_footIdx = robot->getDof("FR_foot_fixed")->getIndexInSkeleton();
  int fl_hipIdx = robot->getDof("FL_hip_joint")->getIndexInSkeleton();
  int fl_thighIdx = robot->getDof("FL_thigh_joint")->getIndexInSkeleton();
  int fl_calfIdx  = robot->getDof("FL_calf_joint")->getIndexInSkeleton();
  //int fl_footIdx = robot->getDof("FL_foot_fixed")->getIndexInSkeleton();
  int rr_hipIdx = robot->getDof("RR_hip_joint")->getIndexInSkeleton();
  int rr_thighIdx = robot->getDof("RR_thigh_joint")->getIndexInSkeleton();
  int rr_calfIdx  = robot->getDof("RR_calf_joint")->getIndexInSkeleton();
  //int rr_footIdx = robot->getDof("RR_foot_fixed")->getIndexInSkeleton();
  int rl_hipIdx = robot->getDof("RL_hip_joint")->getIndexInSkeleton();
  int rl_thighIdx = robot->getDof("RL_thigh_joint")->getIndexInSkeleton();
  int rl_calfIdx  = robot->getDof("RL_calf_joint")->getIndexInSkeleton();
  //int rl_footIdx = robot->getDof("RL_foot_fixed")->getIndexInSkeleton();

  int initPos(0);  // 0 : Home, 1 : Simulation, 2 : Experiment
  Eigen::VectorXd q = robot->getPositions();

std::cout<<"set_initial1.5"<<std::endl;
  switch (initPos) {
    case 0: {
      q[0] = 0.000;
      q[fr_hipIdx] = 0.;
    ...
    case 2: {
      YAML::Node simulation_cfg =
          YAML::LoadFile(THIS_COM "Config/Laikago/SIMULATION.yaml");
    ...
    default:
      std::cout << "[wrong initial pos case] in Laikago/Main.cpp" << std::endl;
  }

std::cout<<"set_initial2"<<std::endl;
...
int main(int argc, char** argv) {
  ...
  dart::dynamics::SkeletonPtr robot = urdfLoader.parseSkeleton(
      THIS_COM "RobotModel/Robot/Laikago/Laikago.urdf");
  ...
  //Eigen::Vector3d gravity(0.0, 0.0, 0.01);
  ...
  // Wrap a worldnode
  // =========================================================================
  osg::ref_ptr<LaikagoWorldNode> node;
  node = new LaikagoWorldNode(world);
