...
template <int DIM, int DEGREE, int NUM_MIDDLE, int CONST_LEVEL_INI,
          int CONST_LEVEL_FIN>
class BS_Basic {
 public:
  BS_Basic()
  ...
  virtual bool SetParam(double* init, double* fin, double** middle_pt,
                        double fin_time) {
    _CalcKnot(fin_time);
 ...
 protected:
  inline void _CalcKnot(double Tf) {
    ...
    // uniform knot sequence for the middle part, #: NumKnot - degree - degree =
    // NumKnot - order - order + 2
  ...
  bool _CurveDerivsAlg1V(double** CK, double u, int d) {
    ...
    for (_k = 0; _k < d + 1; ++_k) _nders[_k] = new double[DEGREE + 1];
  ...
  bool _BasisFunsDers(double** ders, int span, double u, int n) {
    ...
    for (_j = 0; _j <= DEGREE; ++_j) _ndu[_j] = new double[DEGREE + 1];
  ...
  inline double _Left(int i, int j, double u) { return u - Knots_[i + 1 - j]; }

  inline double _Right(int i, int j, double u) { return Knots_[i + j] - u; }
