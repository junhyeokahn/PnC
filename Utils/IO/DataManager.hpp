...
#include <data_manager/DataProtocol.h>
#include <string>
#include <vector>

enum DATA_Type { DOUBLE, INT, VECT2, VECT3, VECT4, QUATERNION, VECT };
...
public:
  ...
  void RegisterData(const void *data_addr, DATA_Type dtype,
                    const std::string &dname, int num_array = 1);

  virtual void run();
  void SaveDataFromValues(double *data, int st_idx, int data_idx);

private:
  void _ShowSendingMessage(const DATA_Protocol::DATA_SETUP &data_setup,
                           const double *data);
