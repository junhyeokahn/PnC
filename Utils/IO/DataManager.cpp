...
void DataManager::SaveDataFromValues(double * data, int st_idx, int data_idx){
    ...
    case VECT2:
      {
        Eigen::Vector2d * tmp_vec2 = (Eigen::Vector2d*)data_addr_vec_[data_idx];
    ...
    case VECT3:
    {
        Eigen::Vector3d * tmp_vec3 = (Eigen::Vector3d*)data_addr_vec_[data_idx];
    ...
    case VECT4:
    {
        Eigen::Vector4d * tmp_vec4 = (Eigen::Vector4d*)data_addr_vec_[data_idx];
    ...
    case QUATERNION:
    {
        Eigen::Quaternion<double>* tmp_quat = (Eigen::Quaternion<double>*)data_addr_vec_[data_idx];
        if(data_array_size_[data_idx] != 4){
            printf("[Warning] Array size incorrect: %i th data\n", data_idx);
        }
    ...
    case VECT:
    {
        Eigen::VectorXd * tmp_vec = (Eigen::VectorXd*)data_addr_vec_[data_idx];
        for(int i(0); i<data_array_size_[data_idx]; ++i){
