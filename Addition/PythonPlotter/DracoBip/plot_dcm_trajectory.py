...
def plot_foot(ax, pos, ori, color, text):
    ...
    ax.scatter(xs=pos[0], ys=pos[1], zs=pos[2], zdir='z', s=50, c=color,
            depthshade=True)
...
def main(args):
    ...
    with open(file, 'r') as stream:
        try:
            ...
            curr_rfoot_contact_pos = np.array(cfg["contact"]["curr_right_foot"]["pos"])
            curr_rfoot_contact_ori = np.array(cfg["contact"]["curr_right_foot"]["ori"])
            curr_lfoot_contact_pos = np.array(cfg["contact"]["curr_left_foot"]["pos"])
            curr_lfoot_contact_ori = np.array(cfg["contact"]["curr_left_foot"]["ori"])
    ...
    com_motion.set_xlabel("x", fontsize=axis_label_size, color=axis_label_color)
    com_motion.set_ylabel("y", fontsize=axis_label_size, color=axis_label_color)
    com_motion.set_zlabel("z", fontsize=axis_label_size, color=axis_label_color)
