...
t = np.genfromtxt(file_path + 'running_time.txt',
                  delimiter='\n',
                  dtype=(float))
...
for i, label in enumerate(task_labels):
    p_d = np.genfromtxt(file_path + task_names[2 * i] + "_des.txt",
                        delimiter=None,
                        dtype=(float))[st_idx:end_idx]
    v_d = np.genfromtxt(file_path + task_names[2 * i + 1] + "_des.txt",
                        delimiter=None,
                        dtype=(float))[st_idx:end_idx]
    p = np.genfromtxt(file_path + task_names[2 * i] + ".txt",
                      delimiter=None,
                      dtype=(float))[st_idx:end_idx]
    v = np.genfromtxt(file_path + task_names[2 * i + 1] + ".txt",
                      delimiter=None,
                      dtype=(float))[st_idx:end_idx]
    tasks_des[label] = [p_d, v_d]
    tasks[label] = [p, v]
est_com_vel = np.genfromtxt(file_path + "com_vel.txt",
                            delimiter=None,
                            dtype=(float))[st_idx:end_idx]
